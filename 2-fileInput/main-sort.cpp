...
void Sort(char names[][MAX_LENGTH], int numOfNames);
...
int main(int argc, char *argv[])
{
    ...
    else
    {
		...
		// Sort(listOfMovies, numberOfMovies);
		...
		// Sort(listOfTheaters, numberOfTheaters);
		// debugging statements to check the above file input is done correctly
		cout << endl;
...
// This function sorts the list of names into ascending alphabetical order 
// Pre-condition: "names" array is filled with a list of names, it could be movie titles or 
//                theater names
//                numOfNames represents the actual number of names in "names" array
// Post-condition: the list of names is sorted in ascending order
void Sort(char names[][MAX_LENGTH], int numOfNames)
{
    bool sorted=false;    // indicates whether additional comparison passes are needed
    int  last=numOfNames-1; // the index of the last item in the remaining part of the array
